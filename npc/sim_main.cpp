...
class Core {
  private:
     static const uint32_t MEM_SIZE = 0x1000;
     ...
     char * coreAddr2Host(uint32_t addr) {
         assert(addr <= MEM_MAX || addr >= MEM_BASE);
         assert(mem != NULL);
	 return (char *) ((uint64_t)addr -(uint64_t) MEM_BASE + (uint64_t)mem);
     }
	...
	for( int i = 0; i < n; i++) {
	 ...
	 return vcore->io_instAddr;
      ...
      uint32_t memAccess(uint32_t addr, int len) {
         char * localAddr = coreAddr2Host(addr); 
	 switch(len) {
