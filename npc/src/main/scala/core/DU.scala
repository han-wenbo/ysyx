...
class DU (enableTestInit : Boolean = false) extends Module {
  ...
  val wEnable  =  if(enableTestInit) io.testEn.getOrElse(false.B) ||  instDecoder.io.ctrl.RegFileWbEnable.en else  instDecoder.io.ctrl.RegFileWbEnable.en
